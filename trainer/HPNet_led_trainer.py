...
from models.model_led_HPNet import Backbone as InitializationModel
...
class Trainer:
	def __init__(self, config):
		...
		self.model_initializer = InitializationModel(
			...
			num_future_steps=20,
			duration=10, # 猜测以帧为单位
			a2a_radius=float('inf'), # 暂时不确定多少合适
			num_attn_layers=3,
			num_modes=20,
			num_heads=8,
			dropout=0.1
	...
	def HPNet_data_preprocess(self, data):
		"""
		data: dict
			pre_motion_3D: torch.Size([32, 11, 10, 2]), [batch_size, num_agent, past_frame, dimension]
			fut_motion_3D: torch.Size([32, 11, 20, 2])
		...
		visible_mask = torch.zeros(num_agents, num_steps, dtype=torch.bool).cuda()
		agent_position = torch.zeros(num_agents, num_steps, 2, dtype=torch.float).cuda()
		agent_heading = torch.zeros(num_agents, num_steps, dtype=torch.float).cuda()
		# agent_velocity = torch.zeros(num_agents, num_steps, 2, dtype=torch.float)
		past_agent_vel_length = torch.zeros(num_agents, num_historical_steps, dtype=torch.float).cuda()
		past_agent_vel_theta = torch.zeros(num_agents, num_historical_steps, dtype=torch.float).cuda()

		initial_pos = data['pre_motion_3D'].cuda()[:, :, -1:]

		past_agent_rel_position = ((data['pre_motion_3D'].cuda() - initial_pos)/self.traj_scale).contiguous().view(-1, 10, 2)
		past_agent_vel = torch.cat((past_agent_rel_position[:, 1:] - past_agent_rel_position[:, :-1], torch.zeros_like(past_agent_rel_position[:, -1:])), dim=1)

		past_agent_vel_length = torch.norm(past_agent_vel, dim=-1)
		past_agent_vel_theta = torch.atan2(past_agent_vel[:, :, 1], past_agent_vel[:, :, 0])

		fut_agent_rel_position = ((data['fut_motion_3D'].cuda() - initial_pos)/self.traj_scale).contiguous().view(-1, 20, 2)
		fut_agent_vel = torch.cat((fut_agent_rel_position[:, 1:] - fut_agent_rel_position[:, :-1], torch.zeros_like(fut_agent_rel_position[:, -1:])), dim=1)
		fut_agent_vel_theta = torch.atan2(fut_agent_vel[:, :, 1], fut_agent_vel[:, :, 0])

		agent_position[:, :10] = past_agent_rel_position
		agent_position[:, 10:] = fut_agent_rel_position
		agent_heading[:, :10] = past_agent_vel_theta
		agent_heading[:, 10:] = fut_agent_vel_theta
		# agent_velocity[:, :10] = past_agent_vel
		# agent_velocity[:, 10:] = fut_agent_vel
		visible_mask = torch.ones(num_agents, num_steps, dtype=torch.bool).cuda() # all agents are visible

		processed_data['agent']['num_nodes'] = num_agents
		processed_data['agent']['position'] = agent_position
		processed_data['agent']['heading'] = agent_heading
		processed_data['agent']['velocity_length'] = past_agent_vel_length
		processed_data['agent']['velocity_theta'] = past_agent_vel_theta
		processed_data['agent']['visible_mask'] = visible_mask
		processed_data['agent']['batch'] = torch.tensor([num_agents]).cuda()
	...
	def _train_single_epoch(self, epoch):
		...
		for data in self.train_loader:
			...
			processed_data = self.HPNet_data_preprocess(data)
			pred_proposal, pred_refine = self.model_initializer(data=processed_data)
			...
			pred_refine = pred_refine[:,-1,:,:,:]
			generated_y = self.p_sample_loop_accelerate(past_traj, traj_mask, pred_refine)
	...
	def _test_single_epoch(self):
		...
		with torch.no_grad():
			for data in self.test_loader:
				...
				pred_traj = self.p_sample_loop_accelerate(past_traj, traj_mask, pred_refine)
